...
static inline int imax(int a, int b) { return std::max(a, b); }
...
static void emitAutoGeneratedWarning(FILE *f)
{
    fprintf(f,
...
static void emitHeaderTop(ZCMGen& zcm, FILE *f, const string& name)
{
    emitAutoGeneratedWarning(f);
...
/** Emit output that is common to every header file **/
static void emitHeaderBottom(ZCMGen& zcm, FILE *f)
{
    fprintf(f, "#ifdef __cplusplus\n");
    fprintf(f, "}\n");
    fprintf(f, "#endif\n");
...
static void emitComment(FILE* f, int indent, const string& comment) {
    if (comment == "")
        return;
...
static void emitHeaderStruct(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    ...
    for (auto& lc : ls.constants) {
        ...
        emitComment(f, 0, lc.comment.c_str());
...
static void emitHeaderPrototypes(ZCMGen& zcmgen, FILE *f, ZCMStruct& ls)
{
    ...
    if (!zcmgen.gopt->getBool("c-no-pubsub")) {
        emit(0, "/**");
    ...
    if(zcmgen.gopt->getBool("c-typeinfo")) {
...
static void emitCStructGetHash(ZCMGen& zcmgen, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCArrayLoopsStart(ZCMGen& zcm, FILE *f, ZCMMember& lm, const string& n, int flags)
{
    if (lm.dimensions.size() == 0)
        return;
...
static void emitCArrayLoopsEnd(ZCMGen& zcm, FILE *f, ZCMMember& lm, const string& n, int flags)
{
    if (lm.dimensions.size() == 0)
        return;
...
static void emitCEncodeArray(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
    ...
    emit(0,"");
    for (auto& lm : ls.members) {
        emitCArrayLoopsStart(zcm, f, lm, "p", FLAG_NONE);

        int indent = 2+imax(0, lm.dimensions.size() - 1);
        ...
        emitCArrayLoopsEnd(zcm, f, lm, "p", FLAG_NONE);
        emit(0,"");
    }
    emit(1,   "}");
    emit(1, "return pos;");
...
static void emitCEncode(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCDecodeArray(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
    ...
    emit(0,"");
    for (auto& lm : ls.members) {
        emitCArrayLoopsStart(zcm, f, lm, "p", lm.isConstantSizeArray() ? FLAG_NONE : FLAG_EMIT_MALLOCS);
...
static void emitCDecodeArrayCleanup(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
    ...
    for (auto& lm : ls.members) {
        ...
        emitCArrayLoopsEnd(zcm, f, lm, "p", lm.isConstantSizeArray() ? FLAG_NONE : FLAG_EMIT_FREES);
        emit(0,"");
    }
    emit(1,   "}");
    emit(1, "return 0;");
...
static void emitCDecode(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCDecodeCleanup(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCEncodedArraySize(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCEncodedSize(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCNumFields(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCStructSize(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCGetField(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCGetTypeInfo(ZCMGen& zcm, FILE *f, ZCMStruct& ls)
{
    string tmp_ = dotsToUnderscores(ls.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCCloneArray(ZCMGen& zcm, FILE *f, ZCMStruct& lr)
{
    ...
    for (auto& lm : lr.members) {
        ...
        emitCArrayLoopsStart(zcm, f, lm, "q", lm.isConstantSizeArray() ? FLAG_NONE : FLAG_EMIT_MALLOCS);
...
static void emitCCopy(ZCMGen& zcm, FILE *f, ZCMStruct& lr)
{
    string tmp_ = dotsToUnderscores(lr.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCDestroy(ZCMGen& zcm, FILE *f, ZCMStruct& lr)
{
    string tmp_ = dotsToUnderscores(lr.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
static void emitCStructPublish(ZCMGen& zcm, FILE *f, ZCMStruct& lr)
{
    string tmp_ = dotsToUnderscores(lr.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
    fprintf(f,
...
static void emitCStructSubscribe(ZCMGen& zcm, FILE *f, ZCMStruct& lr)
{
    string tmp_ = dotsToUnderscores(lr.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();
...
int emitStruct(ZCMGen& zcmgen, ZCMStruct& lr)
{
    string tmp_ = dotsToUnderscores(lr.structname.fullname);
    char *tn_ = (char *)tmp_.c_str();

    string headerName = zcmgen.gopt->getString("c-hpath") + "/" + string(tn_) + ".h";
    string cName      = zcmgen.gopt->getString("c-cpath") + "/" + string(tn_) + ".c";

    if (zcmgen.needsGeneration(lr.zcmfile, headerName)) {
        FILE *f = fopen(headerName.c_str(), "w");
        if (f == NULL)
            return -1;

        emitHeaderTop(zcmgen, f, tn_);
        emitHeaderStruct(zcmgen, f, lr);
        emitHeaderPrototypes(zcmgen, f, lr);

        emitHeaderBottom(zcmgen, f);
        fclose(f);
    }

    // STRUCT C file
    if (zcmgen.needsGeneration(lr.zcmfile, cName)) {
        FILE *f = fopen(cName.c_str(), "w");
        if (f == NULL)
            return -1;
        ...
        fprintf(f, "#include \"%s%s%s.h\"\n",
                zcmgen.gopt->getString("cinclude").c_str(),
                zcmgen.gopt->getString("cinclude").size()>0 ? "/" : "",
                tn_);
        fprintf(f, "\n");

        emitCStructGetHash(zcmgen, f, lr);
        emitCEncodeArray(zcmgen, f, lr);
        emitCEncode(zcmgen, f, lr);
        emitCEncodedArraySize(zcmgen, f, lr);
        emitCEncodedSize(zcmgen, f, lr);

        if(zcmgen.gopt->getBool("c-typeinfo")) {
            emitCStructSize(zcmgen, f, lr);
            emitCNumFields(zcmgen, f, lr);
            emitCGetField(zcmgen, f, lr);
            emitCGetTypeInfo(zcmgen, f, lr);
        }

        emitCDecodeArray(zcmgen, f, lr);
        emitCDecodeArrayCleanup(zcmgen, f, lr);
        emitCDecode(zcmgen, f, lr);
        emitCDecodeCleanup(zcmgen, f, lr);

        emitCCloneArray(zcmgen, f, lr);
        emitCCopy(zcmgen, f, lr);
        emitCDestroy(zcmgen, f, lr);

        if(!zcmgen.gopt->getBool("c-no-pubsub")) {
            emitCStructPublish(zcmgen, f, lr );
            emitCStructSubscribe(zcmgen, f, lr );
        }
...
int emitC(ZCMGen& zcmgen)
{
    ////////////////////////////////////////////////////////////
    // STRUCTS
    for (auto& lr : zcmgen.structs)
        if (emitStruct(zcmgen, lr))
            return -1;
