...
// TODO: Move this into a library routine
    ...
    void emitPythonDecodeOne()
    {
        ...
        for (auto& lm : ls.members) {
            ...
            if (lm.dimensions.size() == 0) {
                if (fmt && lm.type.fullname != "booolean") {
...
// static void
// emit_python_encode_one (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
// {
...
//                 _emit_encode_one (zcm, f, ls, lm, accessor, 2);
//                 g_free(accessor);
//             }
//         } else {
//             _flush_write_struct_fmt (f, struct_fmt, struct_members);
...
//                 }
//                 g_string_append_printf (accessor, "[i%d]", n);
//                 _emit_encode_one (zcm, f, ls, lm, accessor->str, n+3);
//             }
...
// static void
// emit_python_encode (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
// {
...
// static void
// emit_python_init (const zcmgen_t *zcm, FILE *f, zcm_struct_t *lr)
// {
...
// static void
// emit_python_fingerprint (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
// {
...
// static void
// emit_python_dependencies (const zcmgen_t *zcm, FILE *f, zcm_struct_t *ls)
// {
...
//     sprintf (package_dir_prefix, "%s%s", getopt_get_string(zcm->gopt, "ppath"),
//             strlen(getopt_get_string(zcm->gopt, "ppath")) > 0 ?
//             G_DIR_SEPARATOR_S : "");
...
//             if(created_initpy) {
//                 fprintf (init_py_fp, "\"\"\"ZCM package __init__.py file\n"
...
//         zcm_enum_t *le = (zcm_enum_t *) g_ptr_array_index (pc->enums, i);
...
//         fprintf(f, "\"\"\"ZCM type definitions\n"
...
//         emit_python_dependencies (zcm, f, ls);

//         fprintf(f, "class %s(object):\n", ls->structname->shortname);
//         fprintf (f,"    __slots__ = [");
//         for (unsigned int member = 0; member < ls->members->len; member++) {
//             zcm_member_t *lm = (zcm_member_t *) g_ptr_array_index (ls->members, member);
...
//         emit_python_init (zcm, f, ls);
//         emit_python_encode (zcm, f, ls);
//         emit_python_encode_one (zcm, f, ls);
//         emit_python_decode (zcm, f, ls);
//         emit_python_decode_one (zcm, f, ls);
//         emit_python_fingerprint (zcm, f, ls);
//         fclose (f);
//     }
