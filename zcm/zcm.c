...
int zcm_init_from_trans(zcm_t* zcm, zcm_trans_t* zt)
{
    ...
    switch (zt->trans_type) {
        case ZCM_BLOCKING:
                ...
                ret = zcm_blocking_try_create(&zcm->impl, zcm, zt);
                break;
            #else
                zcm->impl = NULL;
                return ZCM_EINVALID;
...
#ifndef ZCM_EMBEDDED
    switch (zcm->type) {
        case ZCM_BLOCKING:    return zcm_blocking_publish(zcm->impl, channel, data, len);
