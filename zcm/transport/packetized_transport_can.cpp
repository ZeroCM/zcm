...
#define ZCM_TRANS_CLASSNAME PacketizedTransportCan
#define MTU (1 << 14)
...
struct ZCM_TRANS_CLASSNAME : public zcm_trans_t
{
    ...
    ~ZCM_TRANS_CLASSNAME()
    {
        ...
        if (soc != -1 && close(soc) < 0) { ZCM_DEBUG("Failed to close"); }
        soc = -1;
    }

    bool good() { return soc != -1 && socSettingsGood; }
    ...
    static size_t get(uint8_t* data, size_t nData, void* usr)
    {
        ...
        if (fromNewFrame < frameDataSize) {
            ...
            me->leftoverOffset = 0;
            memcpy(me->leftoverBuffer, (uint8_t*)frame.data + fromNewFrame,
                   me->leftoverBytes);
        }
    ...
    static size_t sendFrame(const uint8_t* data, size_t nData, void* usr)
    {
        ...
        if (write(me->soc, &frame, sizeof(struct can_frame)) !=
    ...
    size_t get_mtu() { return zcm_trans_get_mtu(this->gst); }
    ...
    static size_t _get_mtu(zcm_trans_t* zt) { return cast(zt)->get_mtu(); }
