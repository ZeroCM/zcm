...
#define ZCM_TRANS_CLASSNAME PacketizedTransportSerial
#define MTU (1 << 14)

using u8  = uint8_t;
using u64 = uint64_t;

struct PacketizedSerialDevice
{
    PacketizedSerialDevice() {}
    ~PacketizedSerialDevice() { close(); }
    ...
    PacketizedSerialDevice(const PacketizedSerialDevice&)            = delete;
...
bool PacketizedSerialDevice::open(const string& port_, int baud, bool hwFlowControl)
{
    if (baud == 0) {
...
void PacketizedSerialDevice::close()
{
    if (isOpen()) {
        ::close(fd);
        fd = 0;
...
int PacketizedSerialDevice::write(const u8* buf, size_t sz)
{
    assert(this->isOpen());
...
int PacketizedSerialDevice::read(u8* buf, size_t sz, u64 timeoutUs)
{
    assert(this->isOpen());
    fd_set fds;
    FD_ZERO(&fds);
...
int PacketizedSerialDevice::convertBaud(int baud)
{
    switch (baud) {
...
struct ZCM_TRANS_CLASSNAME : public zcm_trans_t
{
    ...
    {
        ser.close();
        if (gst) zcm_trans_packetized_serial_destroy(gst);
    }
    ...
    int sendmsg(zcm_msg_t msg)
    {
        if (raw) {
            ...
            return packetized_serial_update_tx(this->gst);
        }
    }
    ...
    int recvmsg(zcm_msg_t* msg, unsigned timeoutMs)
    {
        ...
        if (raw) {
            ...
            do {
                ...
                packetized_serial_update_rx(this->gst);
