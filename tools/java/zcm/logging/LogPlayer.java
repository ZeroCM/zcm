...
public class LogPlayer extends JComponent
{
    ...
    **/
            ...
            try {
                while (!stopflag)
                {
                    ...
                    if (f.enabled && f.outchannel.length() > 0)
                        zcm.publish(f.outchannel, e.data, 0, e.data.length);
    ...
    public static void usage()
    {
        ...
        System.err.println("");
        System.err.println("zcm-logplayer-gui is the Lightweight Communications and Marshalling");
        System.err.println("log playback tool.  It provides a graphical user interface for playing logfiles");
        System.err.println("recorded with zcm-logger.  Features include random access, different playback ");
    ...
    public static void main(String args[])
    {
        ...
        if(System.getProperty("java.vendor").indexOf("Free Software Foundation") >= 0) {
            System.err.println("WARNING: Detected gcj. The ZCM log player is not known to work well with gcj.");
