...
static string makeAccessor(ZCMMember& zm, const string& n, size_t dim)
{
...
static string makeArraySize(ZCMMember& zm, const string& n, size_t dim)
{
    if (zm.dimensions.size() == 0) {
        return "1";
    } else {
...
struct Emit : public Emitter
{
    ...
    Emit(ZCMGen& zcm, ZCMStruct& zs, const string& fname):
...
{
    EmitHeader(ZCMGen& zcm, ZCMStruct& zs, const string& fname):
        Emit{zcm, zs, fname} {}
...
{
    EmitSource(ZCMGen& zcm, ZCMStruct& zs, const string& fname):
        Emit{zcm, zs, fname} {}
    ...
    void emitCArrayLoopsStart(ZCMMember& zm, const string& n, int flags)
    {
        if (zm.dimensions.size() == 0)
            return;
    ...
    void emitCArrayLoopsEnd(ZCMMember& zm, const string& n, int flags)
    {
        if (zm.dimensions.size() == 0)
            return;
    ...
    void emitCNumFields()
    {
        ...
        emit(0,"int %s_num_fields(void)", tn_);
        emit(0,"{");
        emit(1, "return %d;", (int)zs.members.size());
        emit(0,"}");
        emit(0,"");
    }
    ...
    void emitCStructPublish()
    {
        ...
        emit(0, "      }");
        emit(0, "      int status = zcm_publish (zcm, channel, buf, (size_t)data_size);");
        emit(0, "      free (buf);");
...
static int emitStructHeader(ZCMGen& zcm, ZCMStruct& zs, const string& fname)
{
...
static int emitStructSource(ZCMGen& zcm, ZCMStruct& zs, const string& fname)
{
