...
void setupOptionsJulia(GetOpt& gopt)
{
    ...
    gopt.addString(0, "julia-default-pkg", "zcmtypes", "Julia default package, "
                                                       "all types/packages will be inside default");
}
...
static string getAbstractBaseName(const ZCMTypename& zt)
{
    return "_abstract_" + zt.nameUnderscore();
}
...
struct EmitJulia : public Emitter
{
    ...
    EmitJulia(ZCMGen& zcm, ZCMStruct& zs, const string& fname):
        ...
        abstractName(getAbstractBaseName(zs.structname)),
    ...
    void emitAbstractBase()
    {
        const char* an = abstractName.c_str();

        emitComment(0, "Various compatibility fixes to ensure this runs on Julia v0.5 and v0.6");
        emit(0,        "@static if VERSION < v\"0.6.0-\"");
        emitComment(0, "this syntax was deprecated in Julia v0.6. This definition has");
        emitComment(0, "to be wrapped in an include_string to avoid throwing deprecation");
        emitComment(0, "warnings on v0.6. This can be eliminated when v0.5 support is");
        emitComment(0, "dropped.");
        emit(1,            "include_string(\"abstract %s <: ZCM.AbstractZCMType\")", an);
        emit(0,        "else");
        emit(1,            "include_string(\"abstract type %s <: ZCM.AbstractZCMType end\")", an);
        emit(0,        "end");
        emit(0,        "export %s", an);
        emit(0,        "");
    }
    ...
    void emitInstance()
    {
        ...
        emit(0, "export %s", sn);
        emit(0, "type %s <: %s", sn, abstractName.c_str());
        emit(0, "");
    ...
    void emitAbstractConstructor()
    {
        emit(0, "function ZCM._create_abstract_zcmtype(::Type{%s})", abstractName.c_str());
        emit(1,     "return %s()", zs.structname.shortname.c_str());
        emit(0, "end");
        emit(0, "");
    }
...
{
    // RRR: "wasSpecified" is not behaving well ... for little_endian_t it's coming out as
    //      false even though for all others it's true
