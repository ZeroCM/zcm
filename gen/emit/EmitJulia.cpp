...
struct EmitJuliaType : public Emitter
{
    ...
    {
        auto pkgs = StringUtil::split(zs.structname.package, '.');

        emitAutoGeneratedWarning();
        emit(0, "module _%s", zs.structname.shortname.c_str());
        emitStart(0, "__basemodule = module_parent(");
        for (size_t i = 0; i < pkgs.size(); ++i) {
            emitContinue("module_parent(");
        }
        emitContinue("_%s", zs.structname.shortname.c_str());
        for (size_t i = 0; i < pkgs.size(); ++i) {
            emitContinue(")");
        }
        emitEnd(")");
    ...
    void emitModuleEnd()
    {
        emit(0, "end # `module _%s` block", zs.structname.shortname.c_str());
    }
...
struct EmitJuliaPackage : public Emitter
{
    ...
    void emitTypes(const vector<ZCMStruct*>& pkgStructs)
    {
        ...
        for (auto& s : pkgStructs) {
            emit(1, "using _%s", s->structname.shortname.c_str());
        }
        emit(0, "");
    }
