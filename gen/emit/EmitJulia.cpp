...
struct EmitJulia : public Emitter
{
    ...
    void emitGetHash()
    {
        ...
        emit(2, "instance._get_hash_recursive = function(parents::Array{AbstractString})");
        ...
        for (auto& lm : ls.members) {
            if (!ZCMGen::isPrimitiveType(lm.type.fullname)) {
                emit(3, "newparents::Array{AbstractString} = [parents[:]; \"%s\"::String];", sn);
                break;
            }
        }
        ...
        emit(3,     "return Int64(_get_hash_recursive([]))");
        emit(2, "end");
        emit(1, "");
    }
